...
static int file_read(struct Fd *fd, void *buf, u32 n, u32 offset);
static int file_write(struct Fd *fd, const void *buf, u32 n, u32 offset);
...
static int
file_read(struct Fd *fd, void *buf, u32 n, u32 offset)
{
	u32 size;
	struct Filefd *f;
	f = (struct Filefd *)fd;
...
//	starting at 'offset'.
int
read_map(int fdnum, u32 offset, void **blk)
{
	int r;
	u64 va;
	struct Fd *fd;
	...
	if (!((* vpd)[SELF_PD(va)]&VALID) || !((* vpm)[SELF_PM(va)]&VALID) || !((* vpt)[SELF_PT(va)]&VALID)) {
		return -E_NO_DISK;
	}
...
static int
file_write(struct Fd *fd, const void *buf, u32 n, u32 offset)
{
	int r;
	u32 tot;
	struct Filefd *f;
