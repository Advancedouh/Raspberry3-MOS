...
void init_disk() {
    ...
    nbitblock = ROUND(NBLOCK, BIT2BLK) / BIT2BLK;
    nextbno = 2 + nbitblock;
...
void flush_bitmap() {
    ...
    for(i = 0; i < nextbno; ++i) {
        ((u64 *)disk[2+i/BIT2BLK].data)[(i%BIT2BLK)/64] &= ~(1ul<<(i%64));
    }
}
...
void save_block_link(struct File *f, int nblk, int bno)
{
    ...
    if(nblk < NDIRECT) {
        ...
        ((u64 *)(disk[f->f_indirect].data))[nblk] = bno;
    }
}
...
struct File *create_file(struct File *dirf) {
    struct File *dirblk;
    int i, bno, j;
    int nblk = ROUND(dirf->f_size, BY2BLK) / BY2BLK;
	...
	for (i = 0; i < nblk; i++)
	{
		...
		{
			bno = ((u64 *)(disk[dirf->f_indirect].data))[i];
		}
    ...
    printf("address at %lx\n", (struct File *)disk[bno].data);
...
void write_file(struct File *dirf, const char *path) {
    ...
    while((r = read(fd, disk[nextbno].data, n)) > 0) {
        int nbo = next_block(BLOCK_DATA);
        printf("data is at pos %d\n", nbo);
        save_block_link(target, iblk++, nbo);
    }
...
void write_directory(struct File *dirf, char *name) {
    ...
    closedir(fd); // Close file descriptor.
}
