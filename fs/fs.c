...
u64 nbitmap;		// number of bitmap blocks
u64 *bitmap;		// bitmap blocks mapped in memory
...
va_is_mapped(u64 va)
{
	return (((*vpd)[SELF_PD(va)] & VALID) && ((*vpm)[SELF_PM(va)] & VALID) && ((*vpt)[SELF_PT(va)] & VALID));
}

// Overview:
//	Check if this disk block is mapped to a vitrual memory address. (check corresponding `va`)
u64
block_is_mapped(u32 blockno)
{
	u64 va = diskaddr(blockno);
	if (va_is_mapped(va)) {
		return va;
	}
	return 0;
...
int
map_block(u32 blockno)
{
    if (block_is_mapped(blockno) != 0) return 0;
    return syscall_mem_alloc(0, diskaddr(blockno), VALID | PTE_RW | PTE_USER);
}

// Overview:
//	Unmap a block.
void
unmap_block(u32 blockno)
{
	int r;
	u64 addr = block_is_mapped(blockno);
...
int
read_block(u32 blockno, void **blk, u64 *isnew)
{
	u64 va;
	...
	if (block_is_mapped(blockno)) {	//the block is in memory
		...
		syscall_mem_alloc(0, va, VALID | PTE_RW | PTE_USER);
        sd_read(blockno * SECT2BLK, (void *)va, SECT2BLK);
	}
...
write_block(u64 blockno)
{
	...
	va = diskaddr(blockno);
	sd_write(blockno * SECT2BLK, (void *)va, SECT2BLK);

	syscall_mem_map(0, va, 0, va, (VALID | PTE_RW | PTE_LIBRARY | PTE_USER));
}
...
int
block_is_free(u32 blockno)
{
	if (super == 0 || blockno >= super->s_nblocks) {
		return 0;
	}

	if (bitmap[blockno/64] & (1ul << (blockno % 64))) {
		return 1;
	}
...
alloc_block_num(void)
{
	...
	for (blockno = 3; blockno < super->s_nblocks; blockno++) {
		if (bitmap[blockno / 64] & (1ul << (blockno % 64))) {	//the block is free
			bitmap[blockno / 64] &= ~(1ul << (blockno % 64));
            return blockno;
		}
	}
...
read_bitmap(void)
{
	...
	nbitmap = ROUND(super->s_nblocks, BIT2BLK) / BIT2BLK;
	for (i = 0; i < nbitmap; i++) {
		read_block(i + 2, blk, 0);
	}
	
	bitmap = (u64 *)diskaddr(2);
...
int
file_block_walk(struct File *f, u32 filebno, u64 **ppdiskbno, u64 alloc)
{
	int r;
	u64 *ptr;
	void *blk;
...
int
file_map_block(struct File *f, u32 filebno, u32 *diskbno, u64 alloc)
{
	int r;
	u64 *ptr;
...
int
file_clear_block(struct File *f, u32 filebno)
{
	int r;
	u64 *ptr;
...
int
file_get_block(struct File *f, u32 filebno, void **blk)
{
	int r;
	u32 diskbno;
	u64 isnew;
...
int
file_dirty(struct File *f, u32 offset)
{
	int r;
	void *blk;
...
// 	On error return < 0.
int
file_create(char *path, struct File **file, int filetype)
{
	char name[MAXNAMELEN];
	int r;
	struct File *dir, *f;
...
void
file_truncate(struct File *f, u64 newsize)
{
	u32 bno, old_nblocks, new_nblocks;
...
int
file_set_size(struct File *f, u64 newsize)
{
	if (f->f_size > newsize) {
...
{
	// Your code here
	u32 nblocks, bno, diskno;
	int r;

	nblocks = ROUND(f->f_size, BY2BLK) / BY2BLK;
