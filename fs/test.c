...
fs_test(void)
{
	...
	// back up bitmap
	if ((r = syscall_mem_alloc(0, BY2PG, VALID | PTE_RW | PTE_USER)) < 0) {
	...
	user_assert(bits[r / 64] & (1ul << (r % 64)));
	// and is not free any more
	user_assert(!(bitmap[r / 64] & (1ul << (r % 64))));
